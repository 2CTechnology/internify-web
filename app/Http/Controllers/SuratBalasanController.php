...
class SuratBalasanController extends Controller
{
    ...
    public function index()
    {
        ...
        if (auth()->user()->role == 'Admin') {
            $data = AlurMagang::with('kelompok')->with('kelompok.anggota')->with('kelompok.ketua')->with('kelompok.dospem')->with('kelompok.ketua.prodi')->with('kelompok.anggota.prodi')->with('tempatMagang')->whereNotNull('alur_magangs.proposal')->where('alur_magangs.status_proposal', 'diterima')->orderBy('id', 'desc')->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($ketua && $ketua->fcm_token) {
                ...
                $notifier->sendToDevice($ketua->fcm_token, 'Surat Balasan Terbit', 'Surat balasan magangmu sudah diterbitkan. Silakan cek di aplikasi.');
            ...
            return redirect()
                ->back()
                ->withError('Terjadi kesalahan. ' . $e->getMessage());
    ...
    public function tindakLanjut(Request $request)
    {
        ...
        try {
            ...
            if ($ketua && $ketua->fcm_token) {
                ...
                if ($request->status_surat_balasan === 'mengulang') {
                    $notifier->sendToDevice($ketua->fcm_token, 'Surat Balasan Ditolak', 'Surat balasanmu ditolak. Silakan unggah ulang proposal.');
                } else {
                    $notifier->sendToDevice($ketua->fcm_token, 'Surat Balasan Diterima', 'Surat balasanmu diterima. Silahkan menunggu Surat Pelaksanaan Terbit dari admin prodi.');
                }
            }
            DB::commit();
